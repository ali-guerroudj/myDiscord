...
class ChatWindow:
    def __init__(self, channel, server_address, server_port, sender_name):
        ...
        self.sender_name = user_info[1]  # Utiliser le prénom de l'utilisateur
    ...
    def display_previous_messages(self):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM messages WHERE user = %s", (self.sender_name,))
    ...
    def get_user_info(self):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM users WHERE id = 1")  # Remplacer 1 par l'ID de l'utilisateur connecté
...
channel1 = Channel("", True, False)
chat_window1 = ChatWindow(channel1, '127.0.0.1', 5000, "")  # Remplacez par les informations de votre serveur et le nom de l'expéditeur
